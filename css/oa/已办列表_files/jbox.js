...
var jbox = {
	alert: function(msg,handle){
		var submit = function (v, h, f) {//v表示所点的按钮的返回值，h表示窗口内容的jQuery对象，f表示窗口内容里的form表单键值	
			if (v == 'ok'){
		...
		$.jBox.alert(msg, '综合办公信息处理平台',{submit:submit,showClose:false});
	},
	info: function(msg){
		$.jBox.info(msg, '综合办公信息处理平台');
	},
	/**
	 * 弹出页面
	 * w:宽度
	...
	open: function(url,title,w,h){
		if(undefined==w){w=600;}if(undefined==h){h=250;}
		$.jBox.open("iframe:"+url ,title, w, h, { buttons:{},top:'10%' });
	},
	/**
	 * 弹出页面，适用于弹出简单的层
	 * content:如<div>msg</div>
	 * title:标题
	 * w:宽度
	 * h:高度
	 */
	openNormal: function(content,title,w,h){
		if(undefined==w){w=600;}if(undefined==h){h=250;}
		$.jBox.open(content ,title, w, h, { buttons:{} });
	},	
	/**
	 * 关闭jbox弹出框
	 */
	...
	/**
	 * content:提示的内容
	 * icon:内容图标，可选值有'info'、'success'、'warning'、'error'、'loading'，默认值为'info'，当为'loading'时，timeout值会被设置为0，表示不会自动关闭
	 * options:其它参数选项 
	 */
	...
	loading:function(content,options){
		$.jBox.tip(content,'loading',options); 
	}, 
	/**
	 * confirm对话框
	 * content 提示内容
	 ...
	 * handle 确定后回调方法
	 */	
	confirm:function(content,title,handle){
		var submit = function (v, h, f) {//v表示所点的按钮的返回值，h表示窗口内容的jQuery对象，f表示窗口内容里的form表单键值
		    if (v == 'ok'){
		       	if($.isFunction(handle)){
					handle(h,f);
		...
		$.jBox.confirm(content, title, submit);
	}
}
//================================== 为外部提供的方法=================结束