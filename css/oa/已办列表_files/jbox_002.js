    ...
    $.jBox = function (content, options) {
        ...
        var setFade = function () {
            if (options.showFade) {
                ...
                $boxFade.css({
                    position: "absolute",
                    height: options.isTip ? $(document).height(): $window.height(),
        ...
        var positionBox = function () {
            if (options.isMessager) {
		        ...
		        var _ht = parseInt($box.css("height"));
		        var _windowHt = $(window).height();
		        var _level = (_windowHt-_ht)/2;
		        if(_level>100){_level=(_level-100);}
		        var _itop=options.top;
		        if(!isNaN(_level)){_itop=_level;}
                $boxTemp.css({ top: _itop });
                $box.css({
                    position: "absolute",
                    //top: $boxTemp.offset().top + (options.isTip ? $window.scrollTop() : 0),
                    bottom: _windowHt-_itop-_ht,
