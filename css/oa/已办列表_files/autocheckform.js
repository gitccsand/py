 ...
 * Description:
	...
	目前可以使用的类型有:
	NotEmpty			非空	
	NotEmptyAndChksafe  非空且不能含有特殊字符	
	Number				数字		0-9
	Int		    		数字    	1为是数字，0为不是数字
			...
			while (iPosEnd > 0) {				//遍历所有该字段要检查的格式要求
				...
				switch(sCKType){
					...
					case "NotEmptyAndChksafe":
						...
						if(chksafe(objCheckItem.value)==0){
							defTipMsg = objCheckItem.getAttribute("CK_MSG_Chksafe");
		                	if (defTipMsg != null) {
		                    	alert(objCheckItem.CK_MSG_Chksafe);
		                    }else {
		                    	alert(sCKName + "含有特殊字符!");
							}
		                    objCheckItem.focus();
		                    return false;
						}
					...
					case "FloatNew":
						...
						break;
					 case "IsCh":
						 if(objCheckItem.value!="" && isCh(objCheckItem.value)==0){
							 defTipMsg = objCheckItem.getAttribute("CK_MSG_Ch");
								if (defTipMsg != null) {
									alert(defTipMsg);
								} else {
									alert(sCKName + "不能含有汉字!");
								}
								objCheckItem.select();
								return false;
						 }
